---
source: https://mynote11605.tistory.com/4
date: Sun, 9 Aug 2026 20:43:03 +0900
blog: mynote11605
---
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .hero .subtitle {
    font-size: 1.05rem; color: var(--text2); margin-bottom: 6px;
  }
  .hero .meta {
  ...
  .pipe-step .stitle {
    font-weight: 700; font-size: 0.88rem; color: var(--text); margin-bottom: 2px;
  }
  .pipe-step .stool {
  ...
  .gap-card .gleft, .gap-card .gright { font-size: 0.9rem; }
  .gap-card .gleft { color: var(--red); margin-bottom: 6px; }
  .gap-card .gright { color: var(--green); }
  ...
  /* ── SVG 내 텍스트 스타일 (CSS로는 안 먹히니 inline으로) ── */
</style>
...
<section id="how-it-works">
  ...
  <details class="accord" open>
    ...
    <div class="body">
      ...
      <p><code>EXTRA_TEXT</code>에 대화 내용 전체가 텍스트로 담기고, Flutter Dart 코드가 이를 정제해 <strong>YAML frontmatter + 마크다운</strong> 형식으로 저장한다.</p>

      <div class="diagram">
        <svg viewBox="0 0 680 280" width="680" height="280" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <marker id="arrow" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto">
              <path d="M0,0 L8,3 L0,6 Z" fill="#b87333"/>
            </marker>
          </defs>
          <!-- 박스 -->
          <rect x="10" y="30" width="130" height="50" rx="8" fill="var(--surface)" stroke="var(--border)" stroke-width="1.5"/>
          <text x="75" y="52" text-anchor="middle" font-size="12" fill="var(--text2)" font-family="sans-serif">AI 앱 대화 중</text>
          <text x="75" y="68" text-anchor="middle" font-size="10" fill="var(--text3)" font-family="sans-serif">Claude·Grok·ChatGPT</text>

          <line x1="140" y1="55" x2="185" y2="55" stroke="#b87333" stroke-width="1.5" marker-end="url(#arrow)"/>

          <rect x="190" y="30" width="130" height="50" rx="8" fill="var(--surface)" stroke="var(--border)" stroke-width="1.5"/>
          <text x="255" y="52" text-anchor="middle" font-size="12" fill="var(--text2)" font-family="sans-serif">Share Sheet</text>
          <text x="255" y="68" text-anchor="middle" font-size="10" fill="var(--text3)" font-family="sans-serif">ParksyCapture 선택</text>

          <line x1="320" y1="55" x2="365" y2="55" stroke="#b87333" stroke-width="1.5" marker-end="url(#arrow)"/>

          <rect x="370" y="20" width="150" height="70" rx="8" fill="var(--surface)" stroke="#b87333" stroke-width="2"/>
          <text x="445" y="42" text-anchor="middle" font-size="12" font-weight="bold" fill="#b87333" font-family="sans-serif">ACTION_SEND</text>
          <text x="445" y="58" text-anchor="middle" font-size="10" fill="var(--text2)" font-family="sans-serif">EXTRA_TEXT = 전체 대화</text>
          <text x="445" y="74" text-anchor="middle" font-size="10" fill="var(--text3)" font-family="sans-serif">EXTRA_STREAM = 이미지 (미처리)</text>

          <line x1="520" y1="55" x2="565" y2="55" stroke="#b87333" stroke-width="1.5" marker-end="url(#arrow)"/>

          <rect x="570" y="30" width="100" height="50" rx="8" fill="var(--surface)" stroke="var(--border)" stroke-width="1.5"/>
          <text x="620" y="52" text-anchor="middle" font-size="12" fill="var(--text2)" font-family="sans-serif">.md 저장</text>
          <text x="620" y="68" text-anchor="middle" font-size="10" fill="var(--text3)" font-family="sans-serif">ParksyLog_*.md</text>

          <!-- 하단: 결과 파일 -->
          <rect x="180" y="140" width="320" height="36" rx="6" fill="var(--code-bg)" stroke="var(--border)" stroke-width="1"/>
          <text x="340" y="163" text-anchor="middle" font-size="11" fill="var(--green)" font-family="monospace">/sdcard/Download/parksy-logs/ParksyLog_20260725_080708.md</text>

          <line x1="620" y1="80" x2="620" y2="125" stroke="var(--text3)" stroke-width="1" stroke-dasharray="4,3"/>
          <line x1="620" y1="125" x2="500" y2="140" stroke="var(--text3)" stroke-width="1" stroke-dasharray="4,3"/>
          <line x1="180" y1="140" x2="180" y2="125" stroke="var(--text3)" stroke-width="1"/>
          <line x1="180" y1="140" x2="340" y2="140" stroke="var(--text3)" stroke-width="1"/>

          <!-- 주석 -->
          <text x="30" y="215" font-size="10" fill="var(--text3)" font-family="sans-serif">⚠ EXTRA_STREAM(이미지 URI)은 현재 미처리 — 이미지 없는 텍스트-only 캡처</text>
          <text x="30" y="235" font-size="10" fill="var(--text3)" font-family="sans-serif">⚠ 토큰 필터링 없음 — ghp_·sk- 등 민감 문자열이 그대로 저장될 수 있음</text>
        </svg>
      </div>
    </div>
  </details>
  ...
  <details class="accord">
    ...
    <div class="body">
      ...
      <p>이중 저장이 중요한 이유: 폰을 잃어버려도 GitHub에 모든 대화가 남아있다. <strong>플랫폼 독립적</strong> 지식 보존.</p>

      <div class="callout">
        <strong>UID 분석:</strong> 파일 소유자 UID = <code>10264</code> (Android 앱 샌드박스 전용). proot Ubuntu(UID 0)에서는 <strong>읽기 전용</strong> 접근만 가능 — ParksyCapture만 이 디렉토리에 쓸 수 있다. 파이프라인 연동 시 읽기 전용으로 충분.
      </div>
    </div>
  </details>

  <details class="accord">
    <summary>3단계: 발행 (Publish) — GitHub Actions → Telegram → Tistory</summary>
    <div class="body">
      ...
      <div class="diagram">
        <svg viewBox="0 0 680 200" width="680" height="200" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <marker id="arrow2" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto"><path d="M0,0 L8,3 L0,6 Z" fill="#b87333"/></marker>
            <marker id="arrow3" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto"><path d="M0,0 L8,3 L0,6 Z" fill="#3b6e8f"/></marker>
          </defs>

          <rect x="20" y="60" width="120" height="50" rx="8" fill="var(--surface)" stroke="#b87333" stroke-width="2"/>
          <text x="80" y="82" text-anchor="middle" font-size="12" font-weight="bold" fill="#b87333" font-family="sans-serif">git push</text>
          <text x="80" y="98" text-anchor="middle" font-size="10" fill="var(--text3)" font-family="sans-serif">helana_log</text>

          <line x1="140" y1="85" x2="185" y2="85" stroke="#b87333" stroke-width="1.5" marker-end="url(#arrow2)"/>

          <rect x="190" y="55" width="160" height="60" rx="8" fill="var(--surface)" stroke="var(--border)" stroke-width="1.5"/>
          <text x="270" y="75" text-anchor="middle" font-size="12" font-weight="bold" fill="var(--text)" font-family="sans-serif">GitHub Actions</text>
          <text x="270" y="92" text-anchor="middle" font-size="10" fill="var(--text2)" font-family="sans-serif">log_to_telegram.sh</text>
          <text x="270" y="106" text-anchor="middle" font-size="9" fill="var(--text3)" font-family="sans-serif">MD 기본 / --html 선택</text>

          <line x1="350" y1="85" x2="395" y2="85" stroke="#b87333" stroke-width="1.5" marker-end="url(#arrow2)"/>

          <rect x="400" y="55" width="130" height="60" rx="8" fill="var(--surface)" stroke="var(--border)" stroke-width="1.5"/>
          <text x="465" y="75" text-anchor="middle" font-size="12" font-weight="bold" fill="var(--text)" font-family="sans-serif">Telegram</text>
          <text x="465" y="92" text-anchor="middle" font-size="10" fill="var(--text2)" font-family="sans-serif">@helena_logbot</text>
          <text x="465" y="106" text-anchor="middle" font-size="9" fill="var(--text3)" font-family="sans-serif">PART 1~N 청크</text>

          <line x1="530" y1="85" x2="575" y2="85" stroke="#b87333" stroke-width="1.5" marker-end="url(#arrow2)"/>

          <rect x="580" y="60" width="90" height="50" rx="8" fill="var(--surface)" stroke="#b87333" stroke-width="2"/>
          <text x="625" y="82" text-anchor="middle" font-size="12" font-weight="bold" fill="#b87333" font-family="sans-serif">Tistory</text>
          <text x="625" y="98" text-anchor="middle" font-size="10" fill="var(--text3)" font-family="sans-serif">복사+붙여넣기</text>

          <!-- RSS 순환 -->
          <rect x="270" y="140" width="140" height="36" rx="18" fill="none" stroke="#3b6e8f" stroke-width="1.5" stroke-dasharray="6,3"/>
          <text x="340" y="163" text-anchor="middle" font-size="10" fill="#3b6e8f" font-family="sans-serif">RSS 순환 피드백</text>
        </svg>
      </div>
    </div>
  </details>
...
<section id="three-layers">
  ...
  <p>ParksyCapture가 있음으로써 가능해진 지식의 층위. 3층이 모두 있어야 지식이 산다:</p>

  <div class="layers">
    <div class="layer l3">
      <h4>Layer 3 — 발행 (Published)</h4>
      <p>Tistory HTML · GitHub Pages · YouTube · Naver<br><em>"찾아서 배울 수 있는" 층</em></p>
    </div>
    <div class="layer l2">
      <h4>Layer 2 — 정제 (Refined)</h4>
      <p>Fact / Feel / Gap / Fix / Next 구조화된 대화록<br><em>"읽으면 이해되는" 층</em></p>
    </div>
    <div class="layer l1">
      <h4>Layer 1 — 날것 (Raw)</h4>
      <p>ParksyCapture 로그 · 스크린샷 · 음성 메모<br><em>"있기만 해도 가치 있는" 층</em></p>
    </div>
  </div>

  <div class="callout">
    <strong>Layer 3만 있으면?</strong> 과정이 안 보인다. 결과물은 있지만 "왜"가 없다.<br>
    <strong>Layer 1만 있으면?</strong> 검색이 안 된다. 데이터는 있지만 지식이 아니다.<br>
    <strong>3층이 모두 있어야:</strong> 날것 → 정제 → 발행의 흐름으로 <em>살아있는 지식</em>이 된다.
  </div>
</section>
...
<section id="architecture">
  ...
  <p>S21 실기기에서 <code>pm path</code> + <code>unzip</code>으로 추출한 실제 내부 구조:</p>
...
<section id="manual">
  ...
  <details class="accord">
    ...
    <div class="body">
      <ol>
        <li>APK 다운로드 (추후 Play Store 또는 GitHub Releases)</li>
        <li>Android "출처를 알 수 없는 앱" 허용</li>
        <li>설치 완료 → 특별한 설정 불필요</li>
      </ol>
    </div>
  </details>

  <details class="accord">
    <summary>캡처 방법</summary>
    <div class="body">
      <ol>
        <li>Claude·Grok·ChatGPT 앱에서 대화 중</li>
        <li>우측 상단 ⋮ → <strong>공유</strong></li>
        <li>공유 대상 목록에서 <strong>ParksyCapture</strong> 선택</li>
        <li>자동으로 <code>.md</code> 파일 생성 완료!</li>
      </ol>
    </div>
  </details>

  <details class="accord">
    <summary>발행 방법 (표준 파이프라인)</summary>
    <div class="body">
      <ol>
        <li>캡처된 <code>.md</code> 파일을 <code>helana_log/logs/</code>에 git push</li>
        <li>GitHub Actions 자동 트리거 → Telegram <code>@helena_logbot</code>으로 전송</li>
        <li>Telegram에서 PART 1~N 순서대로 전체 복사</li>
        <li>Tistory 글쓰기 → <strong>마크다운 모드</strong> → 붙여넣기 → 발행</li>
        <li>(선택) HTML 모드가 필요하면 <code>--html</code> 플래그 사용</li>
      </ol>
    </div>
  </details>
  ...
  <div class="table-wrap">
    <table>
      ...
      <tr><td>Telegram 봇</td><td><code>@helena_logbot</code> (토큰: .secrets.env)</td></tr>
...
<section id="ecosystem">
  ...
  <div class="pipeline">
    <div class="pipe-step active">
      ...
      <div class="stool">ParksyCapture</div>
    </div>
    <div class="pipe-arrow">→</div>
    <div class="pipe-step">
      <div class="stitle">  STORE</div>
      <div class="stool">helana_log /logs/</div>
    </div>
    <div class="pipe-arrow">→</div>
    <div class="pipe-step">
      <div class="stitle">  REFINE</div>
      <div class="stool">Boss + AI 검토</div>
    </div>
    <div class="pipe-arrow">→</div>
    <div class="pipe-step">
      <div class="stitle">  CONVERT</div>
      <div class="stool">parksy_to_html.py</div>
    </div>
    <div class="pipe-arrow">→</div>
    <div class="pipe-step">
      <div class="stitle">  PUBLISH</div>
      <div class="stool">Tistory·GH Pages·YT·Naver</div>
    </div>
  </div>

  <p><strong>ParksyCapture는 이 전체 파이프라인의 시작점이다.</strong> 이 앱 없이는 어떤 대화도 기록되지 않고, 기록되지 않은 대화는 지식이 될 수 없다. 마치 <em>저장 버튼 없는 워드프로세서</em>로 글을 쓰는 것과 같다. 결과물은 있지만, 어떻게 거기에 도달했는지는 영원히 사라진다.</p>
</section>
...
<section id="intent">
  ...
  <div class="table-wrap">
    <table>
      ...
      <tr><td>log_to_telegram.sh v2.0</td><td><span class="yn-yes">✅ 가동</span></td><td>MD 기본 / HTML 선택 듀얼모드</td></tr>
...
<section id="conclusion">
  ...
  <div style="background: var(--surface); border: 2px solid var(--accent); border-radius: var(--radius); padding: 28px 24px; margin: 20px 0; box-shadow: var(--shadow);">
    <p style="font-size: 1.08rem; font-weight: 700; color: var(--text); margin-bottom: 12px;">
      ParksyCapture는 "생각의 스크린샷" 도구다.
    </p>
    <p style="color: var(--text2); margin-bottom: 8px;">
      코드를 찍는 스크린샷이 아니라, <strong style="color: var(--accent);">AI와의 사고 흐름</strong>을 찍는 도구.
    </p>
    <p style="color: var(--text2); margin-bottom: 8px;">
      그 스크린샷(마크다운 로그)이 3층 구조(날것→정제→발행)를 거쳐<br>
      검색 가능하고, 재사용 가능하고, 발행 가능한 <strong style="color: var(--accent);">지식 자산</strong>이 된다.
    </p>
    <p style="color: var(--text2);">
      AI와 일하면서 <strong style="color: var(--accent);">저장 버튼을 누르지 않는 것</strong>은,<br>
      저장 버튼 없는 워드프로세서로 글을 쓰는 것과 같다.
    </p>
  </div>

  <p style="text-align: center; font-size: 1.05rem; font-weight: 700; color: var(--accent); margin-top: 24px;">
    "어떻게 도달했는가"는 "무엇을 만들었는가"만큼 중요하다.<br>
    더 나은 질문을 하기 위한 재료이기 때문이다.
  </p>
</section>
...
<footer>
  ...
  <p style="margin-top: 4px;">GitHub: <a href="https://github.com/helena751107/helana_log" style="color: var(--accent);">helena751107/helana_log</a> · 변환기: <a href="https://github.com/helena751107/helena-programming" style="color: var(--accent);">helena-programming</a></p>
</footer>

---
원문: https://mynote11605.tistory.com/4
